...
def processo_encaminhar(request, pk):
    """Encaminhar processo para próxima etapa"""
    processo = get_object_or_404(ProcessoInstancia, pk=pk)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            processo.etapa_atual = form.cleaned_data['proxima_etapa']
            processo.usuario_atual = form.cleaned_data['usuario_destino']
            processo.save()
            ...
            LogAuditoria.objects.create(
                ...
                usuario=request.user,
                acao='ENCAMINHAMENTO',
                descricao=f'Processo encaminhado para {processo.etapa_atual.nome} - Usuário: {processo.usuario_atual.get_full_name()}'
            )
            
            messages.success(request, f'Processo encaminhado para {processo.usuario_atual.get_full_name()}!')
            return redirect('processo_detail', pk=pk)
    else:
